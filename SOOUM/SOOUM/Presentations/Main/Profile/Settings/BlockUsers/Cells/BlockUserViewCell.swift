...
class BlockUserViewCell: UITableViewCell {
    
    enum Text {
        static let unBlockUserButtonTitle: String = "차단 해제"
    }
    
    static let cellIdentifier = String(reflecting: BlockUserViewCell.self)
    ...
    let unBlockUserButton = SOMButton().then {
        $0.title = Text.unBlockUserButtonTitle
    ...
    private(set) var model: BlockUserInfo = .defaultValue
    ...
    private func setupConstraints() {
        ...
        self.contentView.addSubview(self.unBlockUserButton)
        self.unBlockUserButton.snp.makeConstraints {
            $0.centerY.equalToSuperview()
    ...
    func setModel(_ model: BlockUserInfo) {
